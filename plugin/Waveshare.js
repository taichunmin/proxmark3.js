(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Pm3Waveshare = factory(global._));
  ...
  class Pm3Waveshare {
    ...
    install (context, pluginOption) {
      ...
      const sendCmd = async ({ ack = null, cmd, data = new Packet() }) => {
        ...
        if (___default["default"].isInteger(ack)) {
          ...
          try {
            ...
            await retry(async () => {
              try {
                ...
                if (!___default["default"].includes(UIDS, selCard?.uid?.utf8)) throw new Error('invalid waveshare NFCTag')
              } catch (err) {
                await sleep(100);
                throw err
              }
